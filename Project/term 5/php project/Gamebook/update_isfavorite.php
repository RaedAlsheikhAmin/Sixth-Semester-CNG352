...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    // Get the game name and new favorite status
    ...
    if ($conn->query($sql) === TRUE) {
        $message = "Game favorite status updated successfully!";
    } else {
...
<head>
    <meta charset="UTF-8">
    <title>Update Favorite - Gamebook</title>
...
<body>
    ...
    <main>
        <h2>Update Favorite Status</h2>
        <form method="post" action="update_isfavorite.php">
            <label for="game_name">Game Name:</label>
            <input type="text" id="game_name" name="game_name" required><br>
            <label for="is_favorite">Favorite Status:</label>
            <select id="is_favorite" name="is_favorite" required>
